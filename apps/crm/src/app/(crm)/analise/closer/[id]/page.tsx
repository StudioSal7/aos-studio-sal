...
import type { CloserScoreBreakdown, CloserExtractedData } from '@repo/commercial/types';

type Props = { params: Promise<{ id: string }> };

function OverallScoreRing({ score }: { score: number }) {
  const color =
    score >= 80 ? 'text-leaf' : score >= 60 ? 'text-wood' : 'text-clay';
  return (
...
export default async function CloserAnalysisDetailPage({ params }: Props) {
  ...
  const breakdown = analysis.scoreBreakdown as CloserScoreBreakdown | null;
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <Card className="flex flex-col items-center justify-center gap-6 p-6">
            {analysis.overallScore !== null && (
              <OverallScoreRing score={analysis.overallScore} />
            )}
