...
| `meta_insights_daily` | Tabela-fato Meta Ads — grão dia × anúncio, UNIQUE(date, ad_id), contagens brutas + `actions_raw` jsonb, dinheiro em cents. Sempre upsert |
| `meta_sync_runs` | Log de execução do sync Meta (janela, trigger, status, linhas, erro) |
| `meta_account_events` | Log MANUAL de mudanças na conta de anúncios (budget/pausa/criativo) — anotado na vista Tendência |
...
pnpm --filter crm meta-probe                                             # sondagem de payload da Graph API (1 chamada, valida o mapper)
pnpm --filter crm meta-backfill -- --since=2026-02-01                    # backfill de insights Meta (janelas de 30d, idempotente)
```
...
- **Pipeline Meta Ads = template Atlas, instância SAL (branch feat/trafego-meta-ads)** — sync read-only da Graph **v25.0** no grão **dia × anúncio** (`level=ad`, `time_increment=1`); banco guarda CONTAGENS BRUTAS (`meta_insights_daily`), todo ratio calculado no código por janela (**Σ/Σ, nunca média das linhas nem cpc/cpm/ctr prontos da API**) em `server/lib/ads-metrics`. Dinheiro em integer cents. Compra/receita por **precedência estrita** `fb_pixel_purchase→purchase→omni_purchase` (find, NUNCA soma — ROAS 2–3x). Upsert por (date, ad_id) com set 100% `sql\`excluded.*\`` (referência de coluna Drizzle no set = no-op silencioso!) incluindo NOMES, chunk 500 (teto 65534 params do postgres-js). Cron 6h SP re-upserta D-7→D-1 (reatribuição da Meta assenta por construção); decisão fecha em D-3. Motor de decisão puro (`ads-decision-engine`): sem_alvo→volume_insuficiente→winner→matar→escalar→iterar + fadiga ortogonal (CPA↑+hook↓+freq↑ WoW simultâneos; dado esparso → false). **Targets null de propósito** até o backfill separar frio/quente (cravar POR SEGMENTO em `apps/crm/src/lib/ads.config.ts` — única superfície client-specific). `/trafego` = 3 vistas URL-driven (`?vista=&segmento=`, seletor de segmento obrigatório, nunca blended) + log manual de eventos da conta. Frequência = PROXY (impressões ÷ Σalcance diário, lower bound — alcance não soma). Página faz **3 queries** por load (pool max:10). Envs: `META_ACCESS_TOKEN` (System User — não expira) + `META_AD_ACCOUNT_ID` (numérico SEM `act_`).
