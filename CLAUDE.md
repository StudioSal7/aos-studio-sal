...
- **Pool de conexões pequeno (`max: 3`) por causa da conexão DIRETA do Supabase na serverless** — a `DATABASE_URL` usa a conexão direta (porta 5432, `max_connections = 60`). Na Vercel, CADA invocação serverless cria seu próprio pool; com `max: 10`, poucas invocações concorrentes estouravam o limite (`53300: sorry, too many clients already`), que a UI via como "Connection closed" / server-side exception intermitente — pior em páginas que disparam muitas queries (dashboard ~23). Medido: `max:10 × 12` invocações = 157 falhas; `max:3 × 12` = 0 falhas; render isolado do dashboard ~4,2s. `packages/db/src/client.ts` usa `max: 3` + `idle_timeout: 20`. **Correção definitiva = trocar a `DATABASE_URL` da Vercel para o POOLER do Supabase (Supavisor transaction mode, porta 6543)** — o código já está pronto (`prepare: false`, exigido pelo pooler); migrations continuam pela direta 5432. Nunca elevar `max` de volta sem antes migrar pro pooler. **Não fazer o dashboard (ou qualquer página) disparar dezenas de queries concorrentes** — preferir queries agrupadas (ex.: `getWeeklyFunnel` usa 3 queries com `FILTER`, não 32).
